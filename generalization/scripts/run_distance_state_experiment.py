...
"""Evaluate an alternative path-distance state design for tabular Q-learning."""
...
from src.encoders import encode_feature_state, encode_generalized_feature_state, encode_path_distance_state
...
def main() -> None:
    output_dir = os.path.join(PROJECT_ROOT, "results", "distance_state_experiment")
    ...
    for encoder_name, encoder in encoders:
        ...
        training_figures = plot_training_curves(
            ...
            title=f"Distance-State Training Curves ({encoder_name})",
            subtitle="Tabular Q-learning across random delivery instances",
        ...
        rollout_figures = plot_delivery_map(
            ...
            subtitle="Alternative state-design comparison on a held-out instance",
    ...
    evaluation_figures = plot_evaluation_summary(
        evaluation_rows,
        title="Alternative State-Design Summary",
        subtitle="Comparing path-distance abstraction against earlier tabular encoders",
        output_path=os.path.join(figures_dir, "evaluation_summary.png"),
    )

    payload = {
        "experiment_name": "distance_state_experiment",
        "generator_config": {
    ...
    output_path = os.path.join(output_dir, "distance_state_summary.json")
