...
"""Run a linear approximate Q-learning experiment on delivery generalization."""
...
from src.encoders import encode_patch3x3_plus_vector, encode_patch3x3_plus_state, encode_path_distance_state
...
def main() -> None:
    output_dir = os.path.join(PROJECT_ROOT, "results", "linear_patch_q_experiment")
    ...
    tabular_encoders: List[Tuple[str, object]] = [
        ("distance_state_tabular", encode_path_distance_state),
        ("patch3x3_plus_tabular", encode_patch3x3_plus_state),
    ]
    tabular_training_config = QLearningConfig(
        episodes=800,
        alpha=0.20,
        gamma=0.98,
        epsilon_start=1.0,
    ...
    linear_encoders: List[Tuple[str, object]] = [
        ("patch3x3_plus_linear", encode_patch3x3_plus_vector),
    ]
    linear_training_config = LinearQLearningConfig(
        episodes=1600,
        alpha=0.01,
        gamma=0.98,
        epsilon_start=1.0,
    ...
    for encoder_name, encoder in tabular_encoders:
        ...
        training_figures = plot_training_curves(
            ...
            subtitle="Tabular Q-learning reference runs for the linear-approximation comparison",
        ...
        rollout_figures = plot_delivery_map(
            ...
            subtitle="Reference rollout for the linear-approximation comparison",
    ...
    for encoder_name, encoder in linear_encoders:
        policy, training_summary = train_linear_q_learning_on_instances(
            instances=[item.instance for item in train_batch],
            encoder=encoder,
            encoder_name=encoder_name,
            config=linear_training_config,
        )
        train_eval = evaluate_policy(
            policy=policy,
            instances=[item.instance for item in train_batch],
        ...
        training_figures = plot_training_curves(
            [episode.to_dict() for episode in training_summary.episodes],
            title=f"Training Curves ({encoder_name})",
            subtitle="Linear approximate Q-learning over vectorized patch features",
            output_path=os.path.join(figures_dir, f"training_curves_{encoder_name}.png"),
        )
        representative_rollout = heldout_eval.rollout_summaries[0]
        heldout_instance = heldout_batch[0]
        rollout_figures = plot_delivery_map(
            ...
            representative_rollout.path,
            title=f"Held-Out Rollout ({encoder_name})",
            subtitle="Linear approximate Q-learning on a held-out instance",
            output_path=os.path.join(figures_dir, f"heldout_rollout_{encoder_name}.png"),
            path_label="Held-Out Rollout",
            show_direction_arrows=True,
            shortest_path_steps=heldout_instance.oracle.optimal_steps,
    ...
    evaluation_figures = plot_evaluation_summary(
        evaluation_rows,
        title="Linear Approximate Q-Learning Summary",
        subtitle="Comparing tabular references against linear Q-learning on vectorized patch features",
        output_path=os.path.join(figures_dir, "evaluation_summary.png"),
    )

    payload = {
        "experiment_name": "linear_patch_q_experiment",
        "generator_config": {
        ...
        "tabular_training_config": {
            "episodes": tabular_training_config.episodes,
            "alpha": tabular_training_config.alpha,
            "gamma": tabular_training_config.gamma,
            "epsilon_start": tabular_training_config.epsilon_start,
            "epsilon_end": tabular_training_config.epsilon_end,
            "epsilon_decay_fraction": tabular_training_config.epsilon_decay_fraction,
            "seed": tabular_training_config.seed,
        },
        "linear_training_config": {
            "episodes": linear_training_config.episodes,
            "alpha": linear_training_config.alpha,
            "gamma": linear_training_config.gamma,
            "epsilon_start": linear_training_config.epsilon_start,
            "epsilon_end": linear_training_config.epsilon_end,
            "epsilon_decay_fraction": linear_training_config.epsilon_decay_fraction,
            "seed": linear_training_config.seed,
        },
        "training_instances": [
            {
    ...
    output_path = os.path.join(output_dir, "linear_patch_q_summary.json")
